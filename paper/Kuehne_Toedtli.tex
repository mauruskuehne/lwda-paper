...
Figure~\ref{fig:UAPs} shows the perturbations generated. In their general structure and appearance, they are similar to the ones reported in~\cite{moosavi-dezfooli_universal_2017-1}. This indicates that deviations from the original results stem from configuration details.
...
In particular, the values of num\_classes\footnote{As given in their code at https://github.com/LTS4/}, the number of class boundaries searched, the number of training epochs and other minor parameters such as random initialisation values for the train-test split, etc. had to be taken from their provided code. We have tried to optimize these parameters using grid searches, but the available compute resources have limited these efforts. In particular, specific choices for the value of num\_classes proved difficult since checking all 1000 training boundaries of ImageNet was infeasible, but higher values improved the fooling performance. 
The strategy taken by the authors of~{\bf cite Dezfooli} might be the crucial missing information.
...
The second section reports results by applying the alternated generation of UAPs and the interpolation method given in eq.~(\ref{eq:interp}) using the value $\lambda=0.05$. Using this configuration, the fooling rates remained essentially unchanged compared to the UAP generated only on VGG-16. 
...
The alternated generation UAP generated a perturbation that achieved slightly higher fooling rates on VGG-16 and ResNet-152 (2.1 and 3.2 absolute percentage point increase). For Inception, the perturbation achieved a fooling rate of 67.4\%. This is 14.6\% below the measured self fooling rate of Inception (81.9\%), but 7.4\% higher than the fooling rate achieved with a VGG-16 model. 
...
\hline
											...
											&	Inception	&	VGG-16		&	ResNet-152	& Average over all networks	\\ \hline
Inception V1 UAP							&	\(81.9\%\)		&	\(15.9\%\)	&	\(15.9\%\)	&xxx	\\
VGG-16 UAP								&	\(59.9\%\)		&	\(58.7\%\)	&	\(38.0\%\)	&xxx	\\
ResNet-152 UAP &	\(54.6\%\)		&	\(31.4\%\)	&	\(73.4\%\)&xxx		\\ \hline
alternated generation of UAPs&	\(67.3\%\)		&	\(60.6\%\)	&	\(41.2\%\)		&xxx\\
linear interpolation (\(\lambda=0.05\))	&	\(59.9\%\)		&	\(58.7\%\)	&	\(38.0\%\)	&xxxx	\\
