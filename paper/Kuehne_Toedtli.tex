...
\SetAlgoLined
{\bf Input}:Data set \(X\), classifiers \(\hat{k_i}\in K,i=1\ldots n\), desired norm \(\left\|.\right\|_p\) of
the perturbation \(\xi\).\;
{\bf Output}: Universal perturbation vector \(v\)\;
Initialize \(\rb_k \gets 0\)\;%, epoch\gets 1\;\;

\While{\textup {Average fooling rate is too low}}{
\ForEach{\(\mathrm{classifier}\) \(k\in K\) }{ 
	\(\Delta \mathbf{r}_k\gets\mathrm{DeepFool} (k)\)\;
} 
\(\rb =\frac{1}{n}\sum_{k=1}^n \Delta  \mathbf{r}_k\)\;
\(\rb\gets \sqrt{\xi}\rb/\left\|\rb\right\|_p\) \;
 }
...
%\begin{figure}
%\includegraphics[width=0.05\textwidth]{./images/own_perturbations.png}
