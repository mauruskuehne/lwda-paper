...
\Return \vb
 \caption{Computation of universal adversarial perturbations for multiple neural networks, variant a. For each image, perturbation updates are computed for all classifiers. }\label{alg:1a}
\end{algorithm}
\end{figure}
...
\Return \vb
 \caption{Computation of universal adversarial perturbations for multiple neural networks, variant b. For each image, perturbation updates are computed for the next classifier in the classifier sequence. Then the sequence is cyclically rotated.}\label{alg:1b}
\end{algorithm}
\end{figure}
...
alternated generation of UAPs, variant b&	\(\mathbf{67.3}\%\)		&	\(\mathbf{60.6}\%\)	&	\(\mathbf{41.2}\%\)		&\(\mathbf{56.4}\)\%\\
...
headings. Tab.~\ref{tab1} gives a summary of all heading levels.
