...
In reproducing these numbers, several insufficiently documented design choices had to be researched. For example, the original results are not stated for a given epoch, but using a stopping condition on the error rate. Values reported here are for epoch 20, at which point the mean value over the last 5 epochs typically varies by less than 0.005. Parameter values taken from and in the notation of~\cite{DeepFool-Moosavi-Dezfooli15} are \(p=\infty\) and \(\xi=10\). We used 10 DeepFool perturbation update {\bf maurus, check!} iterations. The overshoot parameter \(\eta=0.02\) was again taken from~\cite{DeepFool-Moosavi-Dezfooli15}. 
...
The alternated generation of a UAP on both networks (Alg.\ref{alg:3}) generated a perturbation that achieved slightly higher fooling rates on VGG-16 and ResNet-152 {\bf than by training it on VGG-16 alone.?? Maurus, bitte überprüfen}.(with an absolute increase of 2.1 and 3.2 percentage points, respectively). For Inception, the perturbation achieved a fooling rate of 67.4\%. This is 14.6\% below the measured self fooling rate of Inception (81.9\%), but 7.4\% higher than the fooling rate achieved with a VGG-16 model. 
...
Inception V1 UAP							&	\(81.9\%\)		&	\(15.9\%\)	&	{\bf WERT überprüfen!}\(15.9\%\)	&37.9\%	\\
VGG-16 UAP								&	\(59.9\%\)		&	\(58.7\%\)	&	\(38.0\%\)	&52.2\%	\\
ResNet-152 UAP &	\(54.6\%\)		&	\(31.4\%\)	&	\(73.4\%\)&53.1\%		\\ \hline
alternated generation of UAPs&	\(67.3\%\)		&	\(60.6\%\)	&	\(41.2\%\)		&56.4\%\\
linear interpolation (\(\lambda=0.05\))	&	\(59.9\%\){\bf WERTE FALSCH?}		&	\(58.7\%\)	&	\(38.0\%\)	&\%\\
